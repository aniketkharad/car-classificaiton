...
anno_test = {}
with open(path_base + '/anno_test.csv') as testImage:
    testingImages = csv.reader(testImage, delimiter = ',')
    for row in testingImages:
        anno_test[row[0]] = [row[1], row[2], row[3], row[4], row[5]]
# The dictionary of the individual images of testing dataset

imageLoc = path_base + '/car_data/car_data/test/'   #The list imageLoc with class names
...
for class_ in class_names :
    ...
    #     # print(anno_test[eachImage][4], class_names[int(anno_test[eachImage][4])-1])
        r0 = anno_test[eachImage][0]
        r1 = anno_test[eachImage][1]
        r2 = anno_test[eachImage][2]
        r3 = anno_test[eachImage][3]
